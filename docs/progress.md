...
- [x] **Phase 1b (frontend)** — login/register pages (RHF + shared Zod), Zustand in-memory access-token store, httpOnly refresh cookie, silent refresh on load + 401 retry, client-side `(dashboard)` `AuthGate`, logout. Browser flow register → dashboard → reload (silent refresh) → logout → login.
...
- [x] **Phase 9** — browser-level E2E (7 scenarios across desktop and mobile: register→dashboard, login/deep-link→silent-refresh, WO lifecycle, parts-consume→notification loop, RBAC-403, QR continuation, QR asset deep-link), Swagger at `/docs`, error/not-found pages, and a CI Playwright job with artifact upload. Live browser runs exercise the built API and production Next output against isolated services.
- [x] **Phase 10** — idempotent seed script (`db:seed`, demo company + admin/manager/tech + assets/WOs/parts, verified locally + login works), deployment configs (`render.yaml` API+Postgres Blueprint, `vercel.json` web build, and the environment-aware rewrite in `apps/web/next.config.ts`), README rewrite (Quick Start with demo account, screenshots, deployment walk-through, full ADR index 0001–0008). R2 deferred per ADR 0005.
- [x] **Phase 11** — buffer and audit remediation: responsive/focus-trapped mobile navigation, favicon, MTTR correction, authorization/session hardening, immutable domain history, deterministic pagination, meaningful coverage gates, isolated CI services, and expanded API/browser regressions.

## Coverage policy

Critical-path assertions remain the primary quality signal. Coverage is also a
CI regression gate, measured with `all: true` so unimported source files count:
