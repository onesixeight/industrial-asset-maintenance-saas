...
import { acceptIdentity, beginIdentityTransition } from "@/lib/auth/session";
import { validatedRelativePath } from "@/components/auth-gate";
...
function ChangePasswordForm() {
  ...
  const next = validatedRelativePath(search.get("next"));
  ...
  async function onSubmit(values: ChangePasswordRequest) {
    ...
    try {
      const res = await changePasswordApi(values, transition.signal);
      if (await acceptIdentity(res, queryClient, transition)) router.push(next);
    } catch (e) {
      if (transition.signal.aborted) return;
      const status = (e as { status?: number }).status;
      if (status === 401) setServerError("Current password is incorrect.");
      else if (status === 400)
        setServerError("New password does not meet the policy.");
