...
import { locationRequestSchema } from "@iam/shared";
...
export default function LocationsPage() {
  ...
  const saveMutation = useMutation({
    mutationFn: async (input: LocationRequest) => {
      if (editing) return locationsApi.update(editing.id, input);
      return locationsApi.create(input);
    },
    onSuccess: () => {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={openNew}>New location</Button>
      </div>
      {isLoading ? <p className="text-muted-foreground">Loading…</p> : <DataTable columns={columns} rows={data ?? []} empty="No locations yet." />}

      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? "Edit location" : "New location"}>
        <form
