...
const { qrToString } = vi.hoisted(() => ({
  qrToString: vi.fn(async () => "<svg>encoded</svg>"),
}));
vi.mock("qrcode", () => ({ toString: qrToString }));
...
const configStub = {
  get: vi.fn((k: string) =>
    k === "PUBLIC_SCAN_BASE" ? "http://localhost:3000" : undefined,
  ),
};
...
describe("AssetsService", () => {
  it("list maps rows and filters by companyId", async () => {
    ...
    (
      prisma.getClient() as never as { asset: { findMany: typeof findMany } }
    ).asset.findMany = findMany;
    (
      prisma.getClient() as never as {
        asset: { count: ReturnType<typeof vi.fn> };
      }
    ).asset.count = vi.fn().mockResolvedValue(101);
    ...
    expect(out).toMatchObject({ page: 1, pageSize: 50, total: 101 });
    expect(out.items[0].createdAt).toBe("2026-01-01T00:00:00.000Z"); // ISO string, not Date
  ...
  it("get throws NotFound when findFirst returns null (cross-tenant)", async () => {
    const prisma = makePrisma({});
    const svc = new AssetsService(prisma, configStub as never);
    await expect(svc.get("asset-x", OTHER)).rejects.toBeInstanceOf(
      NotFoundException,
    );
  });
  ...
  it("create validates FKs and rejects a foreign-tenant location", async () => {
    ...
    await expect(
      svc.create(
        { name: "X", locationId: "loc-foreign", categoryId: CAT },
        COMPANY,
      ),
  ...
  it("create generates a qrCode and maps dates to ISO", async () => {
    const create = vi
      .fn()
      .mockImplementation((args: { data: { qrCode: string } }) =>
        Promise.resolve(row({ qrCode: args.data.qrCode })),
      );
    const prisma = makePrisma({});
    (
      prisma.getClient() as never as { asset: { create: typeof create } }
    ).asset.create = create;
    const svc = new AssetsService(prisma, configStub as never);
    const out = await svc.create(
      { name: "Pump", locationId: LOC, categoryId: CAT },
      COMPANY,
    );
  ...
  it("remove throws Conflict when work orders or inspections exist", async () => {
    ...
    ).asset.findFirst = vi.fn().mockResolvedValue(row());
    (
      prisma.getClient() as never as {
    ...
    const svc = new AssetsService(prisma, configStub as never);
    await expect(svc.remove("asset-1", COMPANY)).rejects.toBeInstanceOf(
      ConflictException,
    );
  });
  ...
  it("remove maps a P2003 delete race to Conflict", async () => {
    ...
    client.asset.findFirst = vi.fn().mockResolvedValue(row());
    client.asset.delete = vi.fn().mockRejectedValue(p2003);
    const svc = new AssetsService(prisma, configStub as never);
  ...
  it("rotateQr overwrites qrCode (old sticker invalidated)", async () => {
    ...
    (
      prisma.getClient() as never as {
        asset: { findFirst: ReturnType<typeof vi.fn>; update: typeof update };
      }
    ).asset.findFirst = vi.fn().mockResolvedValue(row());
    (
      prisma.getClient() as never as {
  ...
  it("updates asset lifecycle status inside the caller tenant", async () => {
    const update = vi.fn().mockResolvedValue(row({ status: "maintenance" }));
    const prisma = makePrisma({});
    (
      prisma.getClient() as never as {
        asset: {
    ...
    (
      prisma.getClient() as never as {
        asset: { update: typeof update };
      }
    ).asset.update = update;
    const svc = new AssetsService(prisma, configStub as never);

    const result = await svc.updateStatus("asset-1", "maintenance", COMPANY);
  ...
  it("findByQr throws NotFound for unknown / cross-tenant token", async () => {
    const prisma = makePrisma({});
    const svc = new AssetsService(prisma, configStub as never);
    await expect(svc.findByQr("bogus", OTHER)).rejects.toBeInstanceOf(
      NotFoundException,
    );
  });
  ...
  it("getQrSvg returns SVG markup embedding the scan URL", async () => {
    ...
    ).asset.findFirst = vi.fn().mockResolvedValue(row({ qrCode: "abc123" }));
    const svc = new AssetsService(prisma, configStub as never);
    const svg = await svc.getQrSvg("asset-1", COMPANY);
    expect(qrToString).toHaveBeenCalledWith(
      "http://localhost:3000/assets/qr/abc123",
      { type: "svg", errorCorrectionLevel: "M" },
    );
    expect(svg).toBe("<svg>encoded</svg>");
  });
});
