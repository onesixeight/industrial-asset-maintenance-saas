...
describe("Assets CRUD + multi-tenancy + delete guard", () => {
  ...
  it("#2 create with a foreign-tenant locationId → 400", async () => {
    ...
    const res = await request(app.getHttpServer())
      ...
      .send({
        name: "X",
        locationId: otherRefs.locId,
        categoryId: otherRefs.catId,
      });
  ...
  it("#6 rotate changes qrCode; old token 404, new token resolves", async () => {
    ...
    const rotate = await request(app.getHttpServer())
      .post(`/assets/${asset.id}/qr/rotate`)
      .set(auth(admin.accessToken));
    expect(rotate.status).toBe(200);
    expect(rotate.body.qrCode).not.toBe(oldToken);
    const newToken = rotate.body.qrCode;

    const oldScan = await request(app.getHttpServer())
      .get(`/assets/qr/${oldToken}`)
      .set(auth(admin.accessToken));
    expect(oldScan.status).toBe(404);

    const newScan = await request(app.getHttpServer())
      .get(`/assets/qr/${newToken}`)
      .set(auth(admin.accessToken));
  ...
  it("#9 filtered list by status/location/category/search", async () => {
    const admin = await registerAdmin();
    const refs = await seedRefs(admin.user.companyId);
    const maintenanceAsset = await createAsset(
      admin.accessToken,
      refs.locId,
      refs.catId,
    );
    ...
    const statusUpdate = await request(app.getHttpServer())
      .patch(`/assets/${maintenanceAsset.id}/status`)
      .set(auth(admin.accessToken))
      .send({ status: "maintenance" });
    expect(statusUpdate.status).toBe(200);
    expect(statusUpdate.body.status).toBe("maintenance");

    const byLoc = await request(app.getHttpServer())
      .get(`/assets?locationId=${refs.locId}`)
      .set(auth(admin.accessToken));
    expect(byLoc.body).toMatchObject({ total: 2 });
    expect(byLoc.body.items).toHaveLength(2);

    const bySearch = await request(app.getHttpServer())
      .get(`/assets?search=Pump`)
      .set(auth(admin.accessToken));
    expect(bySearch.body).toMatchObject({ total: 2 });
    expect(bySearch.body.items).toHaveLength(2);

    const byStatus = await request(app.getHttpServer())
      .get(`/assets?status=maintenance`)
      .set(auth(admin.accessToken));
    expect(byStatus.body).toMatchObject({ total: 1 });
    expect(byStatus.body.items).toHaveLength(1);
    expect(byStatus.body.items[0].id).toBe(maintenanceAsset.id);
  });
