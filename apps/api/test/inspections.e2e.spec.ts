...
async function createTemplate(token: string, labels: string[]) {
  ...
  if (res.status !== 201)
    throw new Error(
      `createTemplate failed: ${res.status} ${JSON.stringify(res.body)}`,
    );
...
describe("Inspections templates + submissions", () => {
  it("#1 template CRUD (server-generated item ids) + scoped list", async () => {
    ...
    expect(list.body.items[0].id).toBe(tpl.id);

    const get = await request(app.getHttpServer())
      .get(`/inspections/templates/${tpl.id}`)
      .set(auth(admin.accessToken));
    ...
    const del = await request(app.getHttpServer())
      .delete(`/inspections/templates/${tpl.id}`)
      .set(auth(admin.accessToken));
  ...
  it("#3 missing item result → 400; unknown itemId → 400", async () => {
    ...
    const missing = await request(app.getHttpServer())
      ...
      .send({
        assetId,
        templateId: tpl.id,
        results: [{ itemId: tpl.items[0].id, value: "pass" }],
      });
    ...
    const unknown = await request(app.getHttpServer())
      ...
      .send({
        ...
        templateId: tpl.id,
        results: [
          { itemId: "bogus", value: "pass" },
          { itemId: tpl.items[1].id, value: "pass" },
        ],
      });
  ...
  it("#4 submit with foreign-tenant assetId/templateId → 404", async () => {
    ...
    const res = await request(app.getHttpServer())
      ...
      .send({
        ...
        results: [{ itemId: otherTpl.items[0].id, value: "pass" }],
      });
  ...
  it("#6 delete template with inspections → 409; no refs → 204", async () => {
    ...
    await request(app.getHttpServer())
      ...
      .send({
        assetId,
        templateId: withInsp.id,
        results: [{ itemId: withInsp.items[0].id, value: "pass" }],
      })
      .expect(201);
    const blocked = await request(app.getHttpServer())
      .delete(`/inspections/templates/${withInsp.id}`)
      .set(auth(admin.accessToken));
    expect(blocked.status).toBe(409);

    const empty = await createTemplate(admin.accessToken, ["B"]);
    const del = await request(app.getHttpServer())
      .delete(`/inspections/templates/${empty.id}`)
      .set(auth(admin.accessToken));
    expect(del.status).toBe(204);
  });
  ...
  it("#7 cross-tenant inspection/template by id → 404", async () => {
    ...
    const tplX = await request(app.getHttpServer())
      .get(`/inspections/templates/${tpl.id}`)
      .set(auth(other.accessToken));
    expect(tplX.status).toBe(404);
    const inspX = await request(app.getHttpServer())
      .get(`/inspections/${insp.body.id}`)
      .set(auth(other.accessToken));
  ...
  it("#8 RBAC: viewer cannot submit or create template → 403; technician can submit, cannot create template", async () => {
    ...
    // viewer submit → 403
    const vSubmit = await request(app.getHttpServer())
      .post("/inspections")
    ...
    const vTpl = await request(app.getHttpServer())
      .post("/inspections/templates")
      .set(auth(viewer.accessToken))
      .send({ name: "X", items: [{ label: "Y" }] });
    expect(vTpl.status).toBe(403);
    // technician submit → 201
    const tSubmit = await request(app.getHttpServer())
      .post("/inspections")
    ...
    const tTpl = await request(app.getHttpServer())
      .post("/inspections/templates")
      .set(auth(tech.accessToken))
      .send({ name: "X", items: [{ label: "Y" }] });
  ...
  it("#9 filtered list by asset/template/passed", async () => {
    ...
    const byAsset = await request(app.getHttpServer())
      .get(`/inspections?assetId=${assetId}`)
      .set(auth(admin.accessToken));
    ...
    const byPassed = await request(app.getHttpServer())
      .get(`/inspections?passed=true`)
      .set(auth(admin.accessToken));
    expect(byPassed.body).toMatchObject({ total: 1 });
    expect(byPassed.body.items).toHaveLength(1);
    expect(byPassed.body.items[0].passed).toBe(true);
  });
  ...
  it("#10 passed=false inspection in filtered=false, not in filtered=true", async () => {
    ...
    const failed = await request(app.getHttpServer())
      ...
      .send({
        assetId,
        templateId: tpl.id,
        results: [{ itemId: tpl.items[0].id, value: "fail" }],
      });
    expect(failed.body.passed).toBe(false);

    const inFalse = await request(app.getHttpServer())
      .get(`/inspections?passed=false`)
      .set(auth(admin.accessToken));
    expect(inFalse.body).toMatchObject({ total: 1 });
    expect(inFalse.body.items).toHaveLength(1);
    expect(inFalse.body.items[0].passed).toBe(false);
    const inTrue = await request(app.getHttpServer())
      .get(`/inspections?passed=true`)
      .set(auth(admin.accessToken));
    expect(inTrue.body).toMatchObject({ total: 0 });
    expect(inTrue.body.items).toHaveLength(0);
  });
});
